...
namespace Utilities
{
	namespace ModBus
	{
		...
		void ModBusSerial::Query(UINT8 WriteData[], UINT16 WriteCount, UINT8 ReadData[], PUINT32 ReadCount, UINT32 ReadCountRequest, UINT8 DevId)
		{
			CAutoLock al(m_ModBusSerialLock);
			...
			*ReadCount = 0;
			...
			{
				char msg[256];
				sprintf_s(msg, sizeof(msg), "ModBus message too big >256bytes, actual size: %d bytes", wdataSize);
			...
			{
				char msg[256];
				sprintf_s(msg, sizeof(msg), "ModBus message, return size too small <3bytes");
				throw Exc(GetCOMPort(), msg);
			}
			...
			{
				char msg[256];
				sprintf_s(msg, sizeof(msg), "ModBus header, Id fault; request: %d, response: %d", DevId, rdata[0]);
				throw Exc(GetCOMPort(), msg);
			}
			...
			else
			{
				...
				{
					char msg[256];
					sprintf_s(msg, sizeof(msg), "ModBus CRC fault; request: %d, response: %d", crc, crc_answer);
					throw Exc(GetCOMPort(), msg);
				}
			}

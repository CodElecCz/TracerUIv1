...
#define COMM_EVTs	ARRAY_CNT(gs_comm_evt)
		...
		SerialProtocol::SerialProtocol(const char* COMPort, BaudRate Baudrate, DataBits Databits, Parity parity, StopBits Stopbit)
		{
		...
		const char* SerialProtocol::GetLastErrorString(DWORD dwLastError)
		{
		...
		void SerialProtocol::Connect(const char* COMPort, BaudRate Baudrate, DataBits Databits, Parity parity, StopBits Stopbits)
		{
			char	comport[32];
			HANDLE	fh = NULL;
			DCB		dcb;
			...
			if (succeed == FALSE)
			{
				...
				char	err_msg[256];
				DWORD dwLastError = GetLastError();
				sprintf_s(err_msg, sizeof(err_msg), "GetCommTimeouts() error: %d; %s", dwLastError, GetLastErrorString(dwLastError));
				throw err_msg;
			}
			...
			if (succeed == FALSE)
			{
				...
				char	err_msg[256];
				DWORD dwLastError = GetLastError();
				sprintf_s(err_msg, sizeof(err_msg), "SetCommTimeouts() error: %d; %s", dwLastError, GetLastErrorString(dwLastError));
				throw err_msg;
			}
			...
			if (succeed == FALSE)
			{
				...
				char	err_msg[256];
				DWORD dwLastError = GetLastError();
				sprintf_s(err_msg, sizeof(err_msg), "SetupComm() error: %d; %s", dwLastError, GetLastErrorString(dwLastError));
				throw err_msg;
			}
			...
			if (succeed == FALSE)
			{
				...
				char	err_msg[256];
				DWORD dwLastError = GetLastError();
				sprintf_s(err_msg, sizeof(err_msg), "PurgeComm() error: %d; %s", dwLastError, GetLastErrorString(dwLastError));
				throw err_msg;
			}
			...
			if (!m_Overlapped.ReadWriteFile.hEvent)
			{
				...
				char	err_msg[256];
				DWORD dwLastError = GetLastError();
				sprintf_s(err_msg, sizeof(err_msg), "Overlapped.CreateEvent() error: %d; %s", dwLastError, GetLastErrorString(dwLastError));
				throw err_msg;
			}
			...
			if (succeed == FALSE)
			{
				...
				char	err_msg[256];
				DWORD dwLastError = GetLastError();
				sprintf_s(err_msg, sizeof(err_msg), "SetCommMask() error: %d; %s", dwLastError, GetLastErrorString(dwLastError));
				throw err_msg;
			}
		}
		...
		void SerialProtocol::Read(UINT8 ReadData[], PUINT32 ReadCount)
		{
			if (!IsConnected())
		...
		void SerialProtocol::Read(UINT8 ReadData[], PUINT32 ReadCount, UINT32 ReadCountRequest)
		{
			if (!IsConnected())
				Connect(m_COMPort.c_str(), (BaudRate)m_Dcb.BaudRate, (DataBits)m_Dcb.ByteSize, (Parity)m_Dcb.Parity, (StopBits)m_Dcb.StopBits);
		...
		void SerialProtocol::serial_read(UINT8 RadData[], PUINT32 ReadCount)
		{
		...
		void SerialProtocol::Write(UINT8 WriteData[], UINT32 WriteCount)
		{
			if (!IsConnected())
				Connect(m_COMPort.c_str(), (BaudRate)m_Dcb.BaudRate, (DataBits)m_Dcb.ByteSize, (Parity)m_Dcb.Parity, (StopBits)m_Dcb.StopBits);
		...
		void SerialProtocol::serial_write(UINT8 WriteData[], UINT32 WriteCount)
		{
			DWORD l;
			BOOL succeed = WriteFile(	m_Fh,
										WriteData,
										WriteCount,
										&l,
										(m_Overlapped.on) ? &(m_Overlapped.ReadWriteFile) : NULL);
		...
		{
			BOOL succeed = PurgeComm(m_Fh, PURGE_TXABORT | PURGE_RXABORT | PURGE_TXCLEAR | PURGE_RXCLEAR);
			if (succeed == FALSE)
			{
				char	err_msg[256];
				DWORD dwLastError = GetLastError();
		...
		void SerialProtocol::Query(UINT8 WriteData[], UINT32 WriteCount, UINT8 ReadData[], PUINT32 ReadCount, UINT32 ReadCountRequest)
		{
			if (!IsConnected())
				Connect(m_COMPort.c_str(), (BaudRate)m_Dcb.BaudRate, (DataBits)m_Dcb.ByteSize, (Parity)m_Dcb.Parity, (StopBits)m_Dcb.StopBits);
		...
		void SerialProtocol::ols_wait_for_overlapped(LPOVERLAPPED p_overlapped, DWORD *p_len, DWORD timeout)
		{
			BOOL succeed;
		...
		void SerialProtocol::ols_wait_comm_event(DWORD *pmask, UINT32 timeout)
		{
			DWORD l, le;
			BOOL succeed;
